...
const Setpsw = () => {
  ...
  document.title = "Set Password | Ace Batting";
  const { id } = useParams();
...
const userotp = sessionStorage.getItem("userotp")
  const CompareOtp = () => {
    ...
    axios.put(URL.setpassword + "/" + id, emaildata).then((res) => {
      if (res.status === 200) {
            ...
            <Col md={8} lg={6} xl={5}>
              <Card className="overflow-hidden">
                <div className="bg-primary bg-soft">
                  <Row>
                    <Col xs={7}>
                        <h5 className="mt-4 p-3">Create your own login password.</h5>
                    </Col>
...
export default withRouter(Setpsw);
